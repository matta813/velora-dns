...
func (a *Agent) executeUpdate(entry *update.Entry) {
	...
	if err := a.updateService(ctx); err != nil {
...
func (a *Agent) getCurrentDigest(ctx context.Context) string {
	out, err := a.runCompose(ctx, "ps", "-q", a.config.ServiceName)
...
func (a *Agent) pullImage(ctx context.Context) error {
	image := os.Getenv("VELORA_RELEASE_IMAGE")
	if image == "" {
		return fmt.Errorf("VELORA_RELEASE_IMAGE not set")
	}
	out, err := a.runCompose(ctx, "pull", a.config.ServiceName)
	if err != nil {
...
func (a *Agent) updateService(ctx context.Context) error {
	out, err := a.runCompose(ctx, "up", "-d", "--no-deps", a.config.ServiceName)
	if err != nil {
