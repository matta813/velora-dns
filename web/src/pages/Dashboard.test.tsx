...
const snapshot = {
  ...
  config: { query_log: { enabled: true, retention: 0, max_rows: 100, queue_size: 10 }, dns: { listen: [], upstreams: ["127.0.0.1:53"], allowed_clients: [], timeout: 2e9, retries: 0, max_concurrent: 1 }, cache: { max_entries: 100, upstream_ttl: 86400 }, http: { listen: "127.0.0.1:8080", web_dir: "web/dist", allowed_hosts: ["localhost", "127.0.0.1", "::1"] }, log_level: "info" },
...
it("renders bounded top-domain and top-client statistics", async () => {
  ...
  render(withI18n(<Dashboard data={snapshot} history={[]} queryLoggingEnabled />));
